...
- (void)SubmitClientToken:(JS::NativeGoogleContactApi::userDataProps &)userData
                  resolve:(nonnull RCTPromiseResolveBlock)resolve
                   reject:(nonnull RCTPromiseRejectBlock)reject {

  clientID = userData.ClientId();
  redirectUrl = userData.rediectUrl();

  if (clientID != nil && redirectUrl != nil && clientID.length > 0 && redirectUrl.length > 0) {
...
- (void)getContact:(NSString * _Nullable)nextPageToken resolve:(nonnull RCTPromiseResolveBlock)resolve reject:(nonnull RCTPromiseRejectBlock)reject { 
...
if(nextPageToken != NULL)
{
  ...
  Contacts = [str stringByAppendingString:nextPageToken];
...
- (void)getOtherContact:(NSString * _Nullable)nextPageToken resolve:(nonnull RCTPromiseResolveBlock)resolve reject:(nonnull RCTPromiseRejectBlock)reject { 
  if (!clientID || [clientID isEqualToString:@""]) {
      NSNumber *statusCode = @0;
      resolve(@{@"status":statusCode ,@"message":@"Authentication Failed"});
      return;
...
if(nextPageToken != NULL)
{
  ...
  otherContacts = [str stringByAppendingString:nextPageToken];
...
{
    return std::make_shared<facebook::react::NativeGoogleContactApiSpecJSI>(params);
}
